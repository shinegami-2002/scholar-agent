...
class TestRouteQuery:
    ...
    @patch("app.agents.nodes.router.invoke_with_retry")
    def test_paper_search_classification(self, mock_invoke, sample_state):
        """A research query should be classified as 'paper_search'."""
        from app.agents.nodes.router import route_query

        mock_invoke.return_value = AIMessage(content="paper_search")
    ...
    @patch("app.agents.nodes.router.invoke_with_retry")
    def test_general_classification(self, mock_invoke, sample_state):
        """A casual greeting should be classified as 'general'."""
        from app.agents.nodes.router import route_query

        mock_invoke.return_value = AIMessage(content="general")
    ...
    @patch("app.agents.nodes.router.invoke_with_retry")
    def test_fuzzy_paper_classification(self, mock_invoke, sample_state):
    ...
    @patch("app.agents.nodes.router.invoke_with_retry")
    def test_unexpected_response_defaults_to_general(self, mock_invoke, sample_state):
        """If the LLM returns something unexpected, it should default to 'general'."""
        from app.agents.nodes.router import route_query

        mock_invoke.return_value = AIMessage(content="I don't understand")
    ...
    @patch("app.agents.nodes.router.invoke_with_retry")
    def test_preserves_existing_steps(self, mock_invoke, sample_state):
    ...
    @patch("app.agents.nodes.router.invoke_with_retry")
    def test_step_has_duration(self, mock_invoke, sample_state):
